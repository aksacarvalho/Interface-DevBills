...
const Transactions = () => {
  ...
  const handleDelete = async(id: string): Promise<void> => {
    try {
      ...
      console.error(error);
      toast.error("Falha ao deletar Transacão. Por favor, tente novamente.");
    } finally {
      setDeletingId("");
    }
  };
    ...
    <div className="container-app py-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        ...
        <Link to="/transacoes/nova"
          className="bg-primary-500 text-[#051626] font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center hover:bg-primary-500 transition-all"
        >
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <Link to="/transacoes/nova"
              className="w-fit mx-auto mt-6 bg-primary-500 text-[#051626] font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center hover:bg-primary-500 transition-all"
            >
              <Plus className="w-4 h-4 mr-2" />
              Nova Transação
            </Link>
              ...
              <tbody className="divide-y divide-gray-700">
                {filteredTransactions.map((transaction) => (
                  <tr key={transaction.id} className="hover:bg-gray-800">
                    ...
                    <td className="px-3 py-4  whitespace-nowrap">
                      <span className={`${transaction.type === transaction.INCOME ? 'text-primary-500' : 'text-red-500'} text-sm font-semibold`}>
                    ...
                    <td className="px-3 py-4  whitespace-nowrap cursor-pointer">
                      <button 
                        ...
                        className="text-red-400 hover:text-red-400 rounded-full cursor-pointer"
                        disabled={deletingId === transaction.id}
                      >
                        {deletingId === transaction.id ? (
                          <span className="inline-block w-4 h-4 border-2 border-red-500 border-t-transparent rounded-ful animate-spin" />
